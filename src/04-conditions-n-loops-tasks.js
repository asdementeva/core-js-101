...
function getFactorial(n) {
  ...
  return n * getFactorial(n - 1); // Recursive call to calculate factorial
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const lowerBound = a < b ? a : b;
  const upperBound = a < b ? b : a;
...
function isCreditCardNumber(ccn) {
  ...
  const ccnArray = ccnStr.split('').map(Number);
  const len = ccnArray.length;
  let sum = 0;
  let double = false;
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    return 'Radix n is out of range (2-10)';
  }
  if (num === 0) {
    return '0';
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPathDirs.length; i += 1) {
    const currentDir = firstPathDirs[i];
